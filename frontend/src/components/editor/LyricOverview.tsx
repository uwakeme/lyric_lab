  ...
  const stats = useMemo(() => {
    ...
    for (const section of currentSong.lyrics) {
      ...
      for (const line of section.lines) {
        ...
        sectionChars += chars;
        totalLines += 1;
        if (line.adaptedText && line.adaptedText !== line.text) {
          adaptedLines += 1;
        }
    ...
    <div className="rounded-2xl border border-slate-200/80 bg-white/70 p-4 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        {cells.map(cell => (
          ...
          >
            <div className={`text-lg font-semibold leading-tight ${cell.accent}`}>
              {cell.value}
            </div>
      ...
      {stats.sectionStats.length > 0 && (
        <div className="pt-3 border-t border-slate-100">
          <div className="text-xs text-slate-500 mb-1.5">段落</div>
